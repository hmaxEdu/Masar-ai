...
function TimelineBar({ label, start, width, progress, color }: any) {
  return (
    <div className="relative w-full h-8 flex items-center">
      <div 
        className="absolute h-8 rounded-md border border-border/40 bg-card/65 shadow-2xs flex items-center justify-between px-3 overflow-hidden"
      ...
      >
        ...
        <div className="flex items-center gap-2 relative z-10 truncate">
          ...
          <span className="text-xs font-bold text-foreground/90 truncate">{label}</span>
        </div>
        <span className="text-[10px] text-muted-foreground/90 font-mono relative z-10 hidden sm:inline-block">{progress}%</span>
      </div>
    </div>
  );
}
...
function CampaignTimelineVisual() {
  ...
  useEffect(() => {
    const interval = setInterval(() => setPlayhead((p) => (p >= 90 ? 10 : p + 1)), 100);
    return () => clearInterval(interval);
  }, []);

  return (
    <div className="relative w-full h-full min-h-[280px] flex flex-col p-5 bg-muted/20 border-t border-border/40 rounded-b-lg overflow-hidden justify-center">
      <div className="absolute inset-0 bg-grid-white/[0.01] bg-[size:16px_16px] pointer-events-none" />
      
      <div className="flex justify-between text-[10px] font-mono text-muted-foreground/60 px-1 border-b border-border/20 pb-2 mb-4 relative z-10">
        <span>WEEK 1</span>
        <span>WEEK 2</span>
        <span>WEEK 3</span>
        <span>WEEK 4</span>
      ...
      <div 
        ...
        style={{ left: `${playhead}%` }}
      >
        <div className="absolute top-0 -translate-x-1/2 bg-rose-500 text-white font-mono text-[10px] px-1.5 py-0.5 rounded-sm shadow-sm">
          TODAY
        </div>
      </div>
    </div>
...
function AssetCard({ label, format, icon: Icon, cardStep, activeStep }: any) {
  const status = activeStep >= cardStep + 1 
    ? { text: "Approved", class: "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" }
    : activeStep === cardStep 
    ? { text: "Reviewing", class: "bg-pink-500/10 text-pink-500 border-pink-500/20 animate-pulse" }
    : { text: "Drafting", class: "bg-muted/50 text-muted-foreground" };

  return (
    <motion.div layout className="bg-card border border-border/40 rounded-lg p-3 shadow-sm flex items-center justify-between gap-3 w-full max-w-sm">
      <div className="flex items-center gap-3 min-w-0">
        <div className="w-8 h-8 rounded bg-muted/60 flex items-center justify-center text-muted-foreground/80 shrink-0">
          <Icon className="w-4 h-4" />
        </div>
        <div className="flex flex-col min-w-0">
          <span className="text-xs font-bold text-foreground/90 leading-tight truncate">{label}</span>
          <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-semibold mt-0.5">{format}</span>
        </div>
      </div>
      <Badge variant="outline" className={`text-[10px] px-2 py-0.5 rounded-sm uppercase tracking-wider font-bold transition-all duration-300 shrink-0 ${status.class}`}>
        {status.text}
      </Badge>
    </motion.div>
  );
...
function AssetPipelineVisual() {
  ...
  return (
    <div className="relative w-full h-full min-h-[280px] flex flex-col p-5 bg-muted/20 border-t border-border/40 rounded-b-lg overflow-hidden justify-center items-center">
      <div className="absolute inset-0 bg-grid-white/[0.01] bg-[size:16px_16px] pointer-events-none" />
      
      <div className="w-full flex flex-col gap-3 relative z-10 items-center">
      ...
      <AnimatePresence mode="wait">
        {step === 3 && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute bottom-4 bg-emerald-500/10 border border-emerald-500/30 text-emerald-500 text-xs font-bold px-4 py-1.5 rounded-full flex items-center gap-2 shadow-md backdrop-blur-md"
          >
            <CheckCircle2 className="w-4 h-4" /> All assets ready for launch
          </motion.div>
        )}
      </AnimatePresence>
    </div>
      ...
      <main className="pt-24 sm:pt-28 pb-16 relative z-10 max-w-5xl mx-auto px-4 sm:px-6">
        <section className="text-center space-y-4 mb-14 sm:mb-20">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }} className="flex flex-col items-center">
            <Badge variant="outline" className="px-3 py-1 mb-4 text-xs tracking-widest uppercase bg-rose-500/5 text-rose-500 border-rose-500/20 backdrop-blur-md rounded-md">
            ...
            <h1 className="text-3xl sm:text-5xl font-bold tracking-tight text-foreground mb-4 leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-rose-500 via-pink-500 to-amber-500">with total precision.</span>
            </h1>
        ...
        <motion.section initial={{ opacity: 0, scale: 0.98, y: 10 }} whileInView={{ opacity: 1, scale: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} className="rounded-lg border border-rose-500/20 bg-gradient-to-br from-rose-500/5 via-background to-background p-8 sm:p-12 text-center relative overflow-hidden shadow-2xs">
          <div className="relative z-10 space-y-4 max-w-xl mx-auto">
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground">Ready to execute your creative vision?</h2>
            <p className="text-muted-foreground text-sm max-w-sm mx-auto leading-relaxed">Launch elegant marketing workflows with automated status tracking. No more lost designs or delayed assets.</p>
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
              <Button size="sm" className="h-10 px-6 text-sm font-semibold w-full sm:w-auto rounded-md shadow-sm" onClick={() => navigate("/login")}>
                Launch Campaigns <ArrowRight className="ml-1.5 h-4 w-4" />
              </Button>
            </div>
          </div>
        </motion.section>
