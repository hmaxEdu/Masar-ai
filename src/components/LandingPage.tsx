...
import { 
  ArrowRight, Sparkles, KanbanSquare, LineChart, 
  Bot, Zap, Shield, LayoutDashboard, Github,
  ...
  return (
    <div className="absolute bottom-4 right-4 sm:bottom-6 sm:right-6 w-[calc(100%-2rem)] sm:w-80 max-w-sm bg-background/95 backdrop-blur-xl border border-border/60 rounded-2xl shadow-2xl overflow-hidden flex flex-col z-20">
      <div className="p-2 sm:p-3 border-b border-border/50 bg-muted/50 flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-foreground">Masar Agent</span>
      </div>
      <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
        ...
        <div className="flex gap-2 justify-end">
          <div className="bg-primary/10 border border-primary/20 p-2 rounded-lg rounded-tr-none text-[10px] sm:text-xs w-[90%] sm:w-[85%] space-y-2 overflow-hidden shadow-sm">
            ...
            >
              <span className="text-primary font-bold">Executing workflow...</span>
            </motion.div>
            ...
            >
              ...
              <span className="truncate text-foreground font-medium">
                {step === 3 ? "Assigning team members..." : "Team assigned successfully"}
              </span>
            </motion.div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-40 h-40  flex items-center justify-center">
              <img src={Logo} className="w-5 h-5 brightness-0 invert" alt="Masar" />
            </div>
            <span className="font-bold text-xl tracking-tight text-foreground hidden xs:block">Masar</span>
          </div>
          <div className="flex items-center gap-2 sm:gap-4">
            
            <ModeToggle />
            <Separator orientation="vertical" />
            <Button variant="ghost" className="hidden sm:flex transition-colors hover:text-primary hover:bg-muted" onClick={onLoginClick}>
              Sign In
            </Button>
            <Separator orientation="vertical" />
            <Button className="shadow-lg shadow-primary/20 group overflow-hidden relative text-sm sm:text-base px-4 sm:px-6" onClick={onLoginClick}>
              <motion.div
      ...
      <main className="pt-24 sm:pt-32 pb-24 relative z-10">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 text-center space-y-8">
          <div className="max-w-4xl mx-auto flex flex-col items-center">
            ...
            >
              Masar brings real AI automation to the enterprise. Plan projects, predict bottlenecks, and execute tasks with speed, precision, and measurable impact.
            </motion.p>
            ...
            >
              <Button size="lg" className="h-12 sm:h-14 px-6 sm:px-8 text-sm sm:text-base shadow-2xl shadow-primary/25 w-full sm:w-auto group relative overflow-hidden" onClick={onLoginClick}>
                <span className="relative z-10 flex items-center">
                  Start Building Free <ArrowRight className="ml-2 h-4 w-4 sm:h-5 sm:w-5 transition-transform group-hover:translate-x-1" />
                </span>
              </Button>
              <Button size="lg" variant="outline" className="h-12 sm:h-14 px-6 sm:px-8 text-sm sm:text-base bg-card/50 text-foreground border-border/80 backdrop-blur w-full sm:w-auto hover:bg-muted transition-colors" onClick={() => window.open('https://github.com/Tessera-Labs-Masar', '_blank')}>
            ...
            <div className="rounded-xl sm:rounded-2xl border border-border/80 bg-card/70 backdrop-blur-2xl shadow-2xl overflow-hidden ring-1 ring-black/5 dark:ring-white/10">
              ...
              <div className="h-8 sm:h-12 border-b border-border flex items-center px-3 sm:px-4 gap-1.5 sm:gap-2 bg-muted/60">
                ...
                <div className="ml-2 sm:ml-4 flex-1 h-5 sm:h-6 bg-background/50 rounded-md border border-border/50 flex items-center justify-center max-w-xs mx-auto">
                   <span className="text-[9px] sm:text-[10px] text-muted-foreground font-medium font-mono truncate px-2">masar-workspace.local</span>
                </div>
              </div>
              ...
              <div className="aspect-[4/5] sm:aspect-[16/9] bg-gradient-to-br from-background/90 via-background to-muted/20 relative overflow-hidden flex">
                 ...
                 <div className="flex-1 p-3 sm:p-6 flex gap-3 sm:gap-6 overflow-hidden">
                    {[
                      ...
                      <div key={col.name} className={`flex-1 space-y-3 sm:space-y-4 ${col.hiddenOnMobile ? 'hidden sm:block' : 'block'}`}>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 mt-24 sm:mt-40 relative z-20">
          ...
          >
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-4 text-foreground">Built at the frontier of applied AI.</h2>
            <p className="text-muted-foreground text-base sm:text-lg max-w-2xl mx-auto px-2">
              Everything you need to operationalize team workflows, wrapped in a blazing-fast, modern interface.
            </p>
          </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-foreground/10 rounded-md flex items-center justify-center">
              <img src={Logo} className="w-4 h-4 brightness-0 invert opacity-90 dark:opacity-100" alt="Masar" />
            </div>
...
function FeatureCard({ icon, title, description, delay }: { icon: React.ReactNode, title: string, description: string, delay: number }) {
  return (
    <motion.div 
      ...
      className="p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-border/60 bg-card/50 backdrop-blur-md hover:bg-card hover:border-border transition-all shadow-sm"
    >
      <div className="w-12 h-12 sm:w-14 sm:h-14 bg-primary/10 text-primary rounded-xl sm:rounded-2xl flex items-center justify-center mb-5 sm:mb-6 ring-1 ring-primary/20">
        {icon}
      </div>
      <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 tracking-tight text-foreground">{title}</h3>
