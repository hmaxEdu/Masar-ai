...
// ============================================================================
// DYNAMIC VISUAL 1: CI/CD GIT AUTOMATION
// ============================================================================
  ...
  return (
    <div className="relative w-full h-full min-h-[280px] flex flex-col p-4 bg-muted/20 border-t border-border/40 rounded-b-lg overflow-hidden justify-center items-center">
      ...
      <div className="w-full max-w-sm relative z-10 flex flex-col gap-3">
        <div className="flex items-center justify-between bg-card border border-border/40 rounded-lg p-3 shadow-sm">
          <div className="flex items-center gap-2">
            <GitBranch className="w-4 h-4 text-primary" />
            <span className="font-mono text-xs font-bold text-foreground">feature/db-auth</span>
          </div>
          <Badge variant="outline" className={`text-[10px] px-2 py-0.5 rounded-sm uppercase tracking-wider font-bold transition-all ${
        ...
        <div className="bg-card border border-border/40 rounded-lg p-3.5 shadow-md flex flex-col gap-3">
          <div className="flex items-center gap-2 pb-2 border-b border-border/30 text-[10px] uppercase font-bold tracking-wider text-muted-foreground/80">
            <Terminal className="w-3.5 h-3.5" /> Automated Checks
          </div>
          ...
          {[
            ...
            return (
              <div key={i} className="flex items-center justify-between text-sm">
                <span className="text-foreground/90 font-medium text-xs">{check.label}</span>
                {isRun ? (
                  <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                ) : (
                  <div className="w-4 h-4 rounded-full border border-border flex items-center justify-center">
                    <div className="w-1.5 h-1.5 rounded-full bg-primary/25 animate-pulse" />
                  </div>
                )}
              </div>
            );
      ...
      <AnimatePresence>
        {step >= 4 && (
          ...
          >
            <CheckCircle2 className="w-4 h-4" /> PR #108 successfully deployed to main
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
// ============================================================================
// DYNAMIC VISUAL 2: BURNDOWN & SPRINT VELOCITY
// ============================================================================
function SprintBurndownVisual() {
  const [percent, setPercent] = useState(0);

  useEffect(() => {
    const timer = setInterval(() => setPercent((p) => (p >= 100 ? 0 : p + 25)), 1800);
    return () => clearInterval(timer);
  }, []);
  ...
  return (
    <div className="relative w-full h-full min-h-[280px] flex flex-col p-4 bg-muted/20 border-t border-border/40 rounded-b-lg overflow-hidden justify-center items-center">
      ...
      <div className="w-full max-w-sm bg-card border border-border/40 rounded-lg p-4 shadow-md relative z-10 flex flex-col gap-4">
        <div className="flex justify-between items-center border-b border-border/30 pb-3">
          <div className="flex flex-col">
            <span className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground/80">Sprint 24</span>
            <span className="text-sm font-bold text-foreground">Velocity Tracker</span>
          </div>
          <Badge variant="secondary" className="font-mono text-[11px] px-2 py-0.5">84 pts</Badge>
        </div>
        ...
        <div className="h-24 w-full relative flex items-end">
          ...
          <div className="absolute inset-0 flex justify-between px-1 pointer-events-none text-[10px] font-mono text-muted-foreground/50 pt-1">
            <span>D1</span>
            <span>D4</span>
            <span>D7</span>
            <span>D10</span>
        ...
        <div className="grid grid-cols-2 divide-x divide-border/30 border-t border-border/30 pt-3">
          <div className="flex flex-col px-2">
            <span className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground/80">Remaining</span>
            <span className="text-base font-extrabold text-foreground">{100 - percent}%</span>
          </div>
          <div className="flex flex-col px-2 pl-4">
            <span className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground/80">Velocity</span>
            <span className="text-base font-extrabold text-emerald-500">On Track</span>
          </div>
        </div>
      </div>
    </div>
...
export default function EngineeringPage() {
  ...
  return (
    <div className="min-h-screen text-foreground bg-background overflow-x-hidden selection:bg-cyan-500/20 font-sans relative" dir="ltr">
      ...
      {/* Rest of the UI remains identical, layout preserved... */}
      <main className="pt-24 sm:pt-28 pb-16 relative z-10 max-w-5xl mx-auto px-4 sm:px-6">
        <section className="text-center space-y-4 mb-14 sm:mb-20">
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }} className="flex flex-col items-center">
            <Badge variant="outline" className="px-3 py-1 mb-4 text-xs tracking-widest uppercase bg-cyan-500/5 text-cyan-500 border-cyan-500/20 backdrop-blur-md rounded-md">
            ...
            <h1 className="text-3xl sm:text-5xl font-bold tracking-tight text-foreground mb-4 leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 via-primary to-indigo-500">sprint velocity.</span>
            </h1>
        ...
        <motion.section initial={{ opacity: 0, scale: 0.98, y: 10 }} whileInView={{ opacity: 1, scale: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} className="rounded-lg border border-cyan-500/20 bg-gradient-to-br from-cyan-500/5 via-background to-background p-8 sm:p-12 text-center relative overflow-hidden shadow-2xs">
          <div className="relative z-10 space-y-4 max-w-xl mx-auto">
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground">Ready to ship your product roadmaps?</h2>
            <p className="text-muted-foreground text-sm max-w-sm mx-auto leading-relaxed">Groom backlogs, track code events, and improve sprint metrics automatically. Setup takes less than 30 seconds.</p>
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
              <Button size="sm" className="h-10 px-5 text-sm font-semibold w-full sm:w-auto rounded-md shadow-sm" onClick={() => navigate("/login")}>
                Start Sprints <ArrowRight className="ml-1.5 h-4 w-4" />
              </Button>
            </div>
          </div>
        </motion.section>
