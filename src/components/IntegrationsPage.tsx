...
// ============================================================================
// DYNAMIC VISUAL 1: GIT -> MASAR -> SLACK PIPELINE (High-Density)
// ============================================================================
  ...
  return (
    <div className="relative w-full h-full min-h-[280px] flex flex-col p-4 bg-muted/20 border-t border-border/40 rounded-b-lg overflow-hidden mt-3">
      ...
      <div className="flex-1 relative z-10 w-full flex flex-col justify-center items-center gap-4">
        ...
        {/* Step 1: Terminal / GitHub Commit */}
        <motion.div 
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: step >= 1 ? 1 : 0.4, y: step >= 1 ? 0 : 5, scale: step >= 1 ? 1 : 0.98 }}
          className="w-[90%] max-w-[240px] bg-[#0d1117] rounded-md border border-border/30 shadow-sm overflow-hidden flex flex-col self-start"
        >
          <div className="h-5 bg-[#161b22] border-b border-border/30 flex items-center px-2.5 gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full bg-destructive/60" />
            <div className="w-1.5 h-1.5 rounded-full bg-yellow-500/60" />
            <div className="w-1.5 h-1.5 rounded-full bg-green-500/60" />
          </div>
          <div className="p-2.5 font-mono text-[9px] text-muted-foreground/80 leading-normal">
            <span className="text-blue-400">~/masar</span>$ git commit -m "Fix redirect (fixes #412)"
            {step >= 1 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-emerald-400 mt-0.5">
        ...
        {/* Step 2: Masar Task Auto-Resolves */}
        <motion.div 
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: step >= 2 ? 1 : 0, scale: step >= 2 ? 1 : 0.95, y: step >= 2 ? 0 : 5 }}
          className={`w-[90%] max-w-[240px] bg-card rounded-md border shadow-sm p-2.5 flex flex-col gap-1.5 transition-colors duration-500 ${step >= 2 ? 'border-primary/40 shadow-primary/5' : 'border-border/60'}`}
        >
          <div className="flex items-center justify-between">
            <Badge variant="outline" className="text-[8px] px-1 py-0 rounded-sm">TASK-412</Badge>
            <div className={`text-[9px] font-bold px-1.5 py-0.5 rounded-sm flex items-center gap-1 ${step >= 3 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-primary/10 text-primary'}`}>
              {step >= 3 ? "Done" : "In Progress"}
            </div>
          </div>
          <p className="text-[11px] font-bold text-foreground">Fix OAuth redirect loop</p>
        ...
        <motion.div 
          ...
          className="absolute right-1 top-2 w-[190px] bg-card rounded-md border border-border/50 shadow-md p-2.5 flex gap-2.5 z-20"
        >
          <div className="w-6 h-6 rounded bg-[#4A154B] flex items-center justify-center shrink-0">
            <Slack className="w-3.5 h-3.5 text-white" />
          </div>
          <div className="flex flex-col gap-0.5 min-w-0">
            <span className="text-[9px] font-bold text-foreground">Masar Bot</span>
...
// ============================================================================
// DYNAMIC VISUAL 2: SUPABASE REAL-TIME DB SYNC (High-Density)
// ============================================================================
  ...
  return (
    <div className="relative w-full h-full min-h-[280px] flex items-center justify-center p-4 bg-muted/20 border-t border-border/40 rounded-b-lg overflow-hidden mt-3">
      <div className="flex w-full items-center justify-center gap-4 relative z-10">
        ...
        {/* Left: DB JSON Payload */}
        <div className="w-36 sm:w-44 bg-[#1c1c1c] border border-border/40 rounded-md p-2.5 shadow-sm font-mono text-[9px] text-muted-foreground flex flex-col gap-0.5">
          <div className="flex items-center gap-1.5 mb-1.5 text-emerald-400 font-bold border-b border-white/5 pb-1.5">
            <Database className="w-3 h-3" /> postgres_changes
          </div>
        ...
        <div className="relative h-px w-10 sm:w-16 bg-border/60">
          <motion.div 
            ...
            className="absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-emerald-500/25 rounded-full blur-[3px]"
          />
          <motion.div 
            initial={{ left: "0%" }}
            animate={{ left: ["0%", "100%", "0%"] }}
            transition={{ duration: 2, ease: "linear", repeat: Infinity }}
            className="absolute top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-emerald-400 rounded-full"
          />
        </div>
        ...
        <motion.div 
          ...
          className="w-20 sm:w-24 bg-card border border-border/40 shadow-sm rounded-md p-2.5 flex flex-col items-center justify-center gap-1.5 text-center"
        >
          <div className="text-[9px] font-bold text-muted-foreground/80 uppercase tracking-wider">Tasks Done</div>
          <motion.div 
            key={synced ? "sync" : "unsync"}
            initial={{ opacity: 0, y: -5 }}
            animate={{ opacity: 1, y: 0 }}
            className={`text-xl font-black ${synced ? 'text-emerald-500' : 'text-foreground'}`}
          >
            {synced ? "89" : "88"}
          </motion.div>
        </motion.div>
    ...
    <div className="min-h-screen text-foreground bg-background overflow-x-hidden selection:bg-primary/20 font-sans relative" dir="ltr">
      ...
      <main className="pt-24 sm:pt-28 pb-16 relative z-10 max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <section className="text-center space-y-4 mb-14 sm:mb-20">
          ...
          >
            ...
            <h1 className="text-3xl sm:text-5xl font-bold tracking-tight text-foreground mb-4 leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-primary to-cyan-400">
                favorite tools.
              </span>
            </h1>
        ...
        <motion.section 
          ...
          className="grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-5 mb-14 sm:mb-20"
        >
          {/* BENTO 1: GitHub & Slack Workflow */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {integrationsList.map((integration, idx) => (
              <motion.div
                ...
                className="group flex flex-col justify-between rounded-lg border border-border/40 p-4 bg-card/45 backdrop-blur-md hover:bg-card  hover:border-primary/25 transition-all cursor-pointer"
              >
                <div>
                  <div className="flex items-center justify-between mb-3.5">
                    ...
                    <span className="text-[8px] uppercase font-bold tracking-widest text-muted-foreground bg-muted px-1.5 py-0.5 rounded-sm">
                      {integration.category}
                    </span>
                  </div>
                  <h3 className="text-xs font-bold text-foreground mb-1">{integration.name}</h3>
            ...
            <motion.div
              ...
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              transition={{ duration: 0.4, delay: integrationsList.length * 0.05 }}
              className="flex flex-col justify-center items-center text-center rounded-lg border border-dashed border-border p-4 bg-muted/5 hover:bg-muted/15 transition-all cursor-pointer min-h-[150px]"
            >
              <div className="h-7 w-7 rounded-full bg-muted flex items-center justify-center text-muted-foreground mb-2">
                <Plus className="h-3.5 w-3.5" />
              </div>
              <h3 className="text-[11px] font-bold text-foreground mb-0.5">Missing a tool?</h3>
              <p className="text-[10px] text-muted-foreground/80 mb-3">Let us know what we should build next.</p>
              <Button variant="outline" size="sm" className="h-7 text-[10px] font-semibold px-2.5 rounded-md">Request Integration</Button>
            </motion.div>
          </div>
        </section>
        ...
        >
          <div className="relative z-10 space-y-4 max-w-xl mx-auto">
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground">
              Ready to integrate your stack?
            </h2>
            <p className="text-muted-foreground text-xs sm:text-sm max-w-sm mx-auto leading-relaxed">
              Create a workspace, link your tools, and let Masar's AI orchestration handle the heavy lifting.
            </p>
