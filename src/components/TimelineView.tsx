...
export function TimelineView({ projectId, onTaskClick }: TimelineViewProps) {
  ...
  const handleDrag = (taskId: number, e: React.MouseEvent, type: 'start' | 'end' | 'move') => {
    ...
    const onMouseUp = (upEvent: MouseEvent) => {
      const deltaX = upEvent.clientX - initialX;
      let unitsMoved = deltaX / PIXELS_PER_UNIT;
      ...
      if (zoom === 'day') {
        ...
        if (type === 'end' || (type === 'move' && task.finishedAt)) {
          ...
          // If moving a task with no end date, we just move the start
          update.startedAt = addHours(task.startedAt, unitsMoved);
        }
      } else {
        if (type === 'start' || type === 'move') {
      ...
      <div className="flex items-center justify-between p-2 border-b bg-muted/50">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium min-w-[120px] text-center">
            {zoom === 'day' ? format(startDate, 'MMM d, yyyy') :
             zoom === 'week' ? `${format(startDate, 'MMM d')} - ${format(addWeeks(startDate, 1), 'MMM d')}` :
        ...
        <div className="flex items-center gap-1 bg-muted p-1 rounded-md">
          <Button variant={zoom === 'day' ? 'secondary' : 'ghost'} size="sm" className="h-7 text-xs" onClick={() => setZoom('day')}>Day</Button>
          <Button variant={zoom === 'week' ? 'secondary' : 'ghost'} size="sm" className="h-7 text-xs" onClick={() => setZoom('week')}>Week</Button>
          <Button variant={zoom === 'month' ? 'secondary' : 'ghost'} size="sm" className="h-7 text-xs" onClick={() => setZoom('month')}>Month</Button>
        </div>
      </div>
        ...
        >
          ...
          <div className="flex sticky top-0 z-30 bg-card border-b h-[40px]">
            {units.map((unit, i) => (
              <div
                key={i}
                className="border-r h-full flex flex-col items-center justify-center text-[10px] text-muted-foreground shrink-0"
          ...
          <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-0">
            {units.map((_, i) => (
              <div
                key={i}
                className="absolute top-0 bottom-0 border-r border-muted/20"
                style={{ left: i * PIXELS_PER_UNIT }}
              />
            ))}
            {tasks.map((_, i) => (
              <div
          ...
          <svg className="absolute top-0 left-0 w-full h-full pointer-events-none z-10">
            ...
            {dependencies.map(dep => {
              ...
              const fromX = getPosition(fromTask.finishedAt || new Date());
              const fromY = tasks.indexOf(fromTask) * ROW_HEIGHT + HEADER_HEIGHT + ROW_HEIGHT / 2;
              const toX = getPosition(toTask.startedAt);
              const toY = tasks.indexOf(toTask) * ROW_HEIGHT + HEADER_HEIGHT + ROW_HEIGHT / 2;

              // Only draw if within visible range
              if (fromX < -500 || fromX > units.length * PIXELS_PER_UNIT + 500) return null;
            ...
            {tasks.map((task, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className="text-xs font-semibold truncate select-none">{task.title}</span>
                  </div>
